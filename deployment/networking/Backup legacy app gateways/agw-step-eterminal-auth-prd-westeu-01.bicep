param applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name string = 'agw-step-eterminal-auth-prd-westeu-01'
param virtualNetworks_vnet_ep_shared_westeu_01_externalid string = '/subscriptions/c71cfef2-700e-47e5-b810-e34898fd2249/resourceGroups/rg-ep-network-prod-01/providers/Microsoft.Network/virtualNetworks/vnet-ep-shared-westeu-01'
param publicIPAddresses_pip_ep_agw_eterminal_auth_prd_01_externalid string = '/subscriptions/c71cfef2-700e-47e5-b810-e34898fd2249/resourceGroups/rg-ep-step-prod-01/providers/Microsoft.Network/publicIPAddresses/pip-ep-agw-eterminal-auth-prd-01'

resource applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource 'Microsoft.Network/applicationGateways@2024-07-01' = {
  name: applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name
  location: 'westeurope'
  properties: {
    sku: {
      name: 'Standard_Small'
      tier: 'Standard'
      family: 'Generation_1'
      capacity: 1
    }
    gatewayIPConfigurations: [
      {
        name: 'appGatewayIpConfig'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/gatewayIPConfigurations/appGatewayIpConfig'
        properties: {
          subnet: {
            id: '${virtualNetworks_vnet_ep_shared_westeu_01_externalid}/subnets/snet-ep-shared-applicationgateway-westeu-01'
          }
        }
      }
    ]
    sslCertificates: [
      {
        name: 'certwildcard'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/sslCertificates/certwildcard'
        properties: {}
      }
    ]
    authenticationCertificates: [
      {
        name: 'StandardHttpsa58fb1e5-d42b-4853-ae60-bbec3a6570af'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/authenticationCertificates/StandardHttpsa58fb1e5-d42b-4853-ae60-bbec3a6570af'
        properties: {
          data: ''
        }
      }
      {
        name: 'EuroportsCertificate20232024'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/authenticationCertificates/EuroportsCertificate20232024'
        properties: {
          data: ''
        }
      }
      {
        name: 'EuroportsWildcard2024-2025'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/authenticationCertificates/EuroportsWildcard2024-2025'
        properties: {
          data: ''
        }
      }
    ]
    frontendIPConfigurations: [
      {
        name: 'appGwPublicFrontendIp'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/frontendIPConfigurations/appGwPublicFrontendIp'
        properties: {
          privateIPAllocationMethod: 'Dynamic'
          publicIPAddress: {
            id: publicIPAddresses_pip_ep_agw_eterminal_auth_prd_01_externalid
          }
        }
      }
    ]
    frontendPorts: [
      {
        name: 'port_443'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/frontendPorts/port_443'
        properties: {
          port: 443
        }
      }
    ]
    backendAddressPools: [
      {
        name: 'eTerminalauth'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/backendAddressPools/eTerminalauth'
        properties: {
          backendAddresses: [
            {
              fqdn: 'ec524-auth.euroports.com'
            }
          ]
        }
      }
    ]
    backendHttpSettingsCollection: [
      {
        name: 'StandardHttps'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/backendHttpSettingsCollection/StandardHttps'
        properties: {
          port: 443
          protocol: 'Https'
          cookieBasedAffinity: 'Disabled'
          pickHostNameFromBackendAddress: false
          requestTimeout: 20
          authenticationCertificates: [
            {
              id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/authenticationCertificates/EuroportsWildcard2024-2025'
            }
          ]
        }
      }
    ]
    httpListeners: [
      {
        name: 'Public-Listener'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/httpListeners/Public-Listener'
        properties: {
          frontendIPConfiguration: {
            id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/frontendIPConfigurations/appGwPublicFrontendIp'
          }
          frontendPort: {
            id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/frontendPorts/port_443'
          }
          protocol: 'Https'
          sslCertificate: {
            id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/sslCertificates/certwildcard'
          }
          hostNames: []
          requireServerNameIndication: false
          customErrorConfigurations: []
        }
      }
    ]
    urlPathMaps: []
    requestRoutingRules: [
      {
        name: 'Public-Listener'
        id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/requestRoutingRules/Public-Listener'
        properties: {
          ruleType: 'Basic'
          httpListener: {
            id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/httpListeners/Public-Listener'
          }
          backendAddressPool: {
            id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/backendAddressPools/eTerminalauth'
          }
          backendHttpSettings: {
            id: '${applicationGateways_agw_step_eterminal_auth_prd_westeu_01_name_resource.id}/backendHttpSettingsCollection/StandardHttps'
          }
        }
      }
    ]
    probes: []
    rewriteRuleSets: []
    redirectConfigurations: []
    sslPolicy: {
      policyType: 'Custom'
      minProtocolVersion: 'TLSv1_2'
      cipherSuites: [
        'TLS_RSA_WITH_AES_256_CBC_SHA256'
        'TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA384'
        'TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256'
        'TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256'
        'TLS_DHE_RSA_WITH_AES_128_GCM_SHA256'
        'TLS_RSA_WITH_AES_128_GCM_SHA256'
        'TLS_RSA_WITH_AES_128_CBC_SHA256'
        'TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256'
        'TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384'
        'TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA'
        'TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA'
        'TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256'
        'TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA384'
        'TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384'
        'TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA'
        'TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA'
        'TLS_RSA_WITH_AES_256_GCM_SHA384'
        'TLS_RSA_WITH_AES_256_CBC_SHA'
        'TLS_RSA_WITH_AES_128_CBC_SHA'
      ]
    }
    enableHttp2: false
  }
}
