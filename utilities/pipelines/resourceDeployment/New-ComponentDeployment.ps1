                ...
                switch ($deploymentScope) {
                    'resourcegroup' {
                        ...
                        if ($PSCmdlet.ShouldProcess('Resource group level deployment', 'Create')) {
                            if ($validationDeployment) {
                                $res = New-AzResourceGroupDeployment @DeploymentInputs -ResourceGroupName $resourceGroupName -WhatIf
                            } else {
                    ...
                    'subscription' {
                        ...
                        if ($PSCmdlet.ShouldProcess('Subscription level deployment', 'Create')) {
                            if ($validationDeployment) {
                                $res = New-AzSubscriptionDeployment @DeploymentInputs -Location $location -WhatIf
                            } else {
...
.EXAMPLE
New-ComponentDeployment -templateFilePath 'C:/KeyVault/deploy.json' -parameterFilePath 'C:/KeyVault/.parameters/parameters.json' -location 'WestEurope' -resourceGroupName 'aLegendaryRg'

Deploy the deploy.json of the KeyVault module with the parameter file 'parameters.json' using the resource group 'aLegendaryRg' in location 'WestEurope'

.EXAMPLE
New-ComponentDeployment -templateFilePath 'C:/ResourceGroup/deploy.json' -parameterFilePath 'C:/ResourceGroup/.parameters/parameters.json' -location 'WestEurope'

Deploy the deploy.json of the ResourceGroup module with the parameter file 'parameters.json' in location 'WestEurope'
#>
function New-ComponentDeployment {
